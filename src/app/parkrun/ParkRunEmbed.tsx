...
export default function ParkRunEmbed() {
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 grid place-items-center bg-[#86c7e8] p-6 text-center text-[#17324d] [@media_(orientation:landscape)]:hidden">
      ...
      <div
        className="relative min-h-0 flex-1 overflow-hidden border-[4px] border-[#17324d] bg-[#73c5ef] shadow-[0_8px_0_#17324d]"
      ...
      >
        <iframe
          ...
          className="block h-[78vh] min-h-[560px] w-full border-0 max-lg:h-[calc(100svh-118px)] max-lg:min-h-0"
          allow="fullscreen"
        />
      </div>

      <div className="fixed inset-x-0 bottom-2 z-40 mx-auto hidden w-[min(92vw,520px)] items-center justify-between gap-3 px-2 [@media_(orientation:landscape)]:max-lg:flex">
        {controlLabels.map((control) => (
          <button
            key={control.code}
            type="button"
            aria-label={control.text}
            className="min-h-16 flex-1 border-[3px] border-[#17324d] bg-[#ffd85a] font-mono text-3xl font-black text-[#17324d] shadow-[0_4px_0_#17324d] active:translate-y-[3px] active:shadow-[0_1px_0_#17324d]"
            onPointerDown={(event) => {
              event.preventDefault()
          ...
          >
            ...
            <span className="block text-[10px] uppercase tracking-[0.16em]">{control.text}</span>
          </button>
        ))}
      </div>
    </>
